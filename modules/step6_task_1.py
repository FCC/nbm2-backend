...
def joinDataFiles(input_queue, output_queue, message_queue, config, start_time):
    """
    executes task 1 processes which join several geojsons and numprov files
    to create a temporary data file for subsequent processes
    ...
    try:
        ...
        for i in range(config['number_servers']):
            message_queue.put('initial_geojson')
        
        # load the queue with the file types and parameters to be processed
        if continue_run:
            file_list = ['tract','county','not big','big','block']
            continue_run, file_counter = s6f.loadInitialFileQueue(file_list, input_queue, config, start_time)

        # process the results of the output from each worker
        if continue_run:
            continue_run = s6f.processWorkerResults(output_queue, file_counter, start_time)

        # flush the queue so the next process can run
        if continue_run:        
            continue_run = s6f.flushQueue(input_queue, output_queue, config, start_time)

        my_message = """
            INFO - STEP 6 (MASTER): TASK 1 OF 5 - JOIN DATA INTO GEOJSON 
            FILES IS COMPLETE
            """
        ...
        my_message = """
            ERROR - STEP 6 (MASTER): TASK 1 OF 5 - FAILED JOINING DATA INTO 
            GEOJSON FILES
            """
